...
func TestPostsAPI(t *testing.T) {
	...
	if got := posts[0]["slug"]; got != "ai-agents-production-revolution-may-2026" {
		t.Fatalf("first post slug = %q, want May 25 agent revolution post", got)
	}

	if got := posts[1]["slug"]; got != "ai-regulatory-divide-2026" {
		t.Fatalf("second post slug = %q, want May 25 regulatory divide post", got)
	}

	if got := posts[2]["slug"]; got != "2026-05-24-openai-genuine-mathematical-discovery" {
		t.Fatalf("third post slug = %q, want May 24 OpenAI discovery post", got)
	}
}
